import {
    ...
    return (
      ...
      >
        <Picture preload={lcp}>
          ...
          />
          <img
            class="w-full h-[640px] max-w-[1920px]"
            loading={lcp ? "eager" : "lazy"}
            src={desktop}
            alt={alt}
          />
        </Picture>
        <div className="flex justify-center bottom-[100px] relative">
        <div className="w-[150px] bg-white rounded flex justify-center] ">
                <div>
                    <p className="font-montserrat text-black font-semibold">{title}</p>
                </div>
            </div>
            </div>
      </a>
  ...
  function Dots({ images, interval = 0 }: Props) {
    return (
      <>
        ...
        />
        <ul id="nossos-servicos" class="carousel justify-center col-span-full z-10 row-start-4 full-phone:relative full-phone:top-6 full-phone:h-7">
          {images?.map((_, index) => (
  ...
  function Carouselphytospa(props: Props) {
    const id = useId();
    ...
    return (
      <div
        id={id}
        class="relative grid grid-cols-[48px_1fr_48px] sm:grid-cols-[120px_1fr_120px] grid-rows-[1fr_48px_1fr_64px] sm:min-h-min mt-[40px] mb-[40px] lg:ml-[40px] lg:mr-[40px]"
      >
  ...
  export default Carouselphytospa;