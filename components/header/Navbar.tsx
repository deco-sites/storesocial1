...
function Navbar({
  ...
  return (
    <>
      ...
      <div class="cs-min-desktop:hidden">
        <div class="mobile-header-container overcontent flex items-center justify-between border-b border-solid border-gray-1 w-full px-6 full-tablet:px-4 full-tablet:py-4 gap-6">
          ...
          <div class="flex justify-end gap-6">
            <div class="search-button-bottom">
              <FloatingHeaderSearchButton />
            </div>
      ...
      <div class="full-tablet:hidden header-middle overcontent z-40 overflow-hidden flex justify-between items-center border-b border-t border-gray-1 w-full px-[64px] py-[23px] bg-white">
        <div
          class={`flex ${
            logoPosition === "left"
        ...
        <div class="flex-none relative flex items-center justify-end gap-6 col-span-1">
          ...
          {!buttons?.hideAccountButton && (
            <a
              ...
              aria-label="Account"
            >
              <div class="flex items-center justify-center text-gray-4 hover:text-primary">
                <Icon id="Account" size={25} class="text-inherit" />
              </div>
            </a>
          )}
      ...
      <div>
        <ul
          class={`full-tablet:hidden flex relative justify-between col-span-1 px-[64px] border-b border-gray-1 bg-white ${
            logoPosition === "left" ? "justify-center" : "justify-start"
          }`}
        >
          {items.map((item) => <NavItem item={item} />)}
          <div class="hidden fixed top-0 left-0 w-full h-full peer-hover:flex bg-black/35 backdrop-blur-sm -z-10" />
        </ul>
      </div>
      ...
      <div class="floating-header-desktop max-w-[1920px] mx-auto full-tablet:hidden fixed w-full border-b border-gray-1 top-0 -z-10 opacity-0">
        <div class="floating-header-container relative px-16 sm-desktop:px-8 flex justify-between items-center bg-white">
          ...
          <ul
            class={`flex justify-between w-full col-span-1 px-[64px] ${
              logoPosition === "left" ? "justify-center" : "justify-start"
            }`}
          >
            {items.map((item) => <NavItem item={item} />)}
