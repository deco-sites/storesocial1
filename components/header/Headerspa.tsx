...
import Navbarspa from "./Navbarspa.tsx";
...
function Headerspa({ searchbar, navItems = [], infoTopbar, fsTopBar, cupom, logo, floatingLogo, mobileLogo, logoPosition, buttons, ecButtons, device }: SectionProps<typeof loader>) {
    const platform = usePlatform();
    const items = navItems ?? [];
    return (<>
      <header>
        <Drawers menu={{ items }} searchbar={searchbar} platform={platform} ecButtons={ecButtons} fsTopBar={fsTopBar}/>
        <div class="header-main bg-base-100 w-full">
          <HeightHeader />
          ...
          <Navbarspa device={device} items={items} searchbar={searchbar && { ...searchbar, platform }} logo={logo} fsTopBar={fsTopBar && { ...fsTopBar }} floatingLogo={floatingLogo} mobileLogo={mobileLogo} logoPosition={logoPosition} buttons={buttons} cupom={cupom && { ...cupom }}/>
        </div>
        <HelmetHead />
      </header>
    </>);
