...
function FilterRange(
  ...
  const rangemin = useSignal(Number(!urlBrowser?.includes('filter.price') ? minValue : currentMinFacet));
  const rangemax = useSignal(Number(!urlBrowser?.includes('filter.price') ? maxValue : currentMaxFacet));
  ...
  function draw(splitvalue: number) {
    if (
      ...
      min.current.style.width = `${Math.floor(
        thumbsize +
        ((splitvalue - minValue) /
          (maxValue - minValue)) *
      ...
      max.current.style.width = `${Math.floor(
        thumbsize +
        ((maxValue - splitvalue) /
          (maxValue - minValue)) *
        (dataValue.value.rangewitdh - 2 * thumbsize),
  ...
  return (
    <div class="flex flex-col">
      ...
      <div class="flex justify-between items-center mt-1">
        <output class="font-arial text-xs font-normal text-gray-6">
